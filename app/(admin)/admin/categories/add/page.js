...
export default function AddCategoryPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = e2?.message || "Failed to create category";
      setError(msg);
      toast.error(msg);
    } finally {
      setSaving(false);
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="pt-6 flex gap-4">
            <button disabled={saving} type="submit" className="flex-1 py-4 bg-indigo-600 text-white rounded-xl font-bold uppercase tracking-widest hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all disabled:opacity-70">
